...
class GraphConvolution(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        # Custom, get adj matrix for hand joints from config
        self.adjmat = torch.sparse_coo_tensor(
            torch.tensor(ADJ_INDICES).T,
            torch.tensor(ADJ_MAT_VALUE),
            size=(NUM_JOINTS, NUM_JOINTS),
        ).to(device)
